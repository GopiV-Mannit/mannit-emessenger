...
function Message({ selectedMenu, setSelectedMenu }) {
  ...
  useEffect(() => {
    ...
    if (globalFilter == '') {
      // If empty, load user data
      loadUserData();
      setRowSelection({});
    } else {
      // If not empty, filter data based on globalFilter
      try {
        //   const filteredData = data.filter((row) =>
        //   row && row.name && row.name.toLowerCase().includes()
        // );
      ...
      // const originalData = data.filter(item => item.name.toLowerCase().includes(globalFilter.toLowerCase()));
      // const filteredData = originalData.filter(item => {
      //   // Add a guard clause to check if 'item.name' is defined
